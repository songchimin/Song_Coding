...
//			}
	...
	@RequestMapping("/chat_position")	//관리자 채팅 버튼 위치 기억
	public void chat_position(HttpServletRequest request, HttpServletResponse response, Model model)
			throws ServletException, IOException{
		
		String chat_left = request.getParameter("chat_left");
		String chat_top = request.getParameter("chat_top");
		
		System.out.println(chat_left +" " +chat_top);
		
		HttpSession session = null;
		session = request.getSession();
		
		session.setAttribute("chat_left",chat_left);
		session.setAttribute("chat_top",chat_top);
	}
	
	
	@RequestMapping("/chat_box_position")	//관리자 채팅 버튼 위치 기억
	public void chat_box_position(HttpServletRequest request, HttpServletResponse response, Model model)
			throws ServletException, IOException{
		...
		session.setAttribute("chat_box_left",chat_left);
		session.setAttribute("chat_box_top",chat_top);
	}
}
