...
public class ChallengeController2 {
	...
	public void public_create(HttpServletRequest request, @RequestParam("title") String title,
			...
			@RequestParam("detail") String detail, @RequestParam("host") String host) {

		System.out.println("create");
		dao.PublicChallenge(title, category, type, detail, host);
	}
	...
	public String comment_insert(HttpServletRequest request, Model model,
		...
		for (int i = 0; i < commentlist.size(); i++) {
			...
			likeExist[i] = dao.CommentLikeExist(member_id,
					commentlist.get(i).getChallenge_num(), commentlist.get(i).getComment_num());
			System.out.println(commentlist.get(i).getComment_num() + " : " + likeExist[i]);
		}
	...
	public String Public_Comment_list(Model model, @RequestParam("member_id") String member_id) {
	...
	public String Public_Comment_like(Model model, @RequestParam("challenge_num") int challenge_num,
		...
		if (dao.CommentLikeExist(member_id, challenge_num, comment_num) == 0) {
			...
			dao.CommentRankUpdate(challenge_num, comment_num);
		...
		for (int i = 0; i < commentlist.size(); i++) {
			...
			// 여기서 한번
			likeExist[i] = dao.CommentLikeExist(member_id, commentlist.get(i).getChallenge_num(),
					commentlist.get(i).getComment_num());
			System.out.println(commentlist.get(i).getComment_num() + " : " + likeExist[i]);
		}
	...
	public void private_create(HttpServletRequest request, @RequestParam("title") String title,
		...
		if (dao.ChallengeExist(title, category, type, frequency, start, end, fee, time, detail, first_image,
				host) == 0) {
			dao.PrivateChallenge(title, category, type, frequency, start, end, fee, time, detail, first_image, host);
		} else {
			dao.PrivateUpdate(title, category, type, frequency, start, end, fee, time, detail, first_image, host);
		}
	}

	@RequestMapping("/private_list")
	public String Private_Challenge_List(Model model, @RequestParam("id") String id) {
	...
	public String Private_Challenge_SearchCode(Model model, @RequestParam("code") String code) {
