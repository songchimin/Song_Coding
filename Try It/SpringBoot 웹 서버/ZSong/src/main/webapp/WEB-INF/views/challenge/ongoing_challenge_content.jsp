...
<script type="text/javascript">
	...
	function approval(num){
		var result = confirm('해당 트라잇을 승인 하시겠습니까?');
...
// 						alert(result);
					...
					for(var i = 0 ; i < result.length; i++){
						addimage(result[i][0].value, result[i][1].value, result[i][2].value, result[i][3].value, result[i][4].value, i);
		//					alert(result[i][0].value +"    "+ result[i][1].value);	
					}
				}
			});
	...
	function addimage(title, comment, image, time, nickname, i){

		$('#image').append('<div class="col-md-4" id='+i+'>' +
				'<div class="card">' +
				'<img style="height:150px;" src="/Feed/'+image+'" class="card-img-top" alt="...">' +
				'<div class="card-body">' +
				'<h5 class="card-title">'+title+ '('+ nickname+')</h5>' +
    ...
    <nav class="col-md-2 d-none d-md-block bg-light sidebar" id="side">
      <div class="sidebar-sticky" style="margin-top: 80px;" id="chase">
        <ul class="nav flex-column">
          ...
          <li class="nav-item" style="margin-bottom: 15px;">
            <a class="nav-link" href="#" onclick="ongoing()" style="color: red;">
...
<!-- 		게시판 위  -->
    ...
    <tr style="color: white;">
      <th scope="col" style="vertical-align:middle; font-size: 15pt;" colspan="6">트라잇 상세정보</th>
    </tr>
...
<!-- 					<button class="btn btn-outline-primary" type="button" onclick="postpone();" >보류</button> -->
					<button class="btn btn-outline-primary" type="button" onclick="javascript:window.location='ongoing_challenge?page=<%= session.getAttribute("cpage") %>';" >목록보기</button>
				</td>
			</tr>
			<tr>
				<td colspan="4"></td>
...
//   };
    ...
    var firebaseConfig = {
    apiKey: "AIzaSyBrkygC-nZL1CjhMtSEyh6ylShMLvAnmik",
    authDomain: "singupin-dd242.firebaseapp.com",
    databaseURL: "https://singupin-dd242.firebaseio.com",
    projectId: "singupin-dd242",
    storageBucket: "singupin-dd242.appspot.com",
    messagingSenderId: "175859706137",
    appId: "1:175859706137:web:91ee59eab6a612edecfd88",
    measurementId: "G-0HSM9NQ4MS"
  };
...
// 		var messageRef1 = database.ref("chat");

		var messageRef = database.ref("chat/"+'${challenge.challenge_num}');
